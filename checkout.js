$(document).ready(function() {
    ...
    function createCheckoutProductCard(obj) {
        ...
        var imagelinktag = document.createElement('a') ;  
        productImg.classList.add('checkout-product-img');
        productImg.src = obj.preview; 
        imagelinktag.href = 'details.html?p='+obj.id ; 
        imagelinktag.appendChild(productImg) ; 
        firstInnerDiv.appendChild(imagelinktag);
        ...
        quantityinc.onclick= function(){ 
            $(quantityinc).addClass('bigger');
            setTimeout(function() {
                $(quantityinc).removeClass('bigger');
            }, 200)  
            if (obj.count<50){ 
            obj.count +=1 ;    
            quantity.innerHTML = "x" + obj.count;
        ...
        quantitydec.onclick=()=>{ 
            $(quantitydec).addClass('bigger');
            setTimeout(function() {
                $(quantitydec).removeClass('bigger');
            }, 200)  
            if(obj.count>1) {  
              obj.count--;    
        ...
        delbtn.onclick = ()=>{ 
            $(delbtn).addClass('bigger');
            setTimeout(function() {
                $(delbtn).removeClass('bigger');
            }, 200)  
          card.remove(); 
          deletedata(obj.id) ; 
          totalamt(obj);  
    ...
    $('#btn-place-order').click(function() { 
        $('#btn-place-order').addClass('bigger');
        setTimeout(function() {
            $('#btn-place-order').removeClass('bigger');
        }, 100)
        var productl = window.localStorage.getItem('product-list');
        productl = productl === null || productl === '' ? [] : productl;
        productl = productl.length > 0 ? JSON.parse(productl) : []; 
